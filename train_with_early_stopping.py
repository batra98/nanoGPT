"""
Training script with early stopping support.
Monitors validation loss and stops training if no improvement is seen.
Based on train.py but adds early stopping logic.
"""
...
if wandb_log and master_process:
    import wandb
    # Use run name as ID so orchestration script can resume the same run
    wandb.init(project=wandb_project, name=wandb_run_name, id=wandb_run_name, config=config, resume='allow')
...
while True:
    ...
    if iter_num % eval_interval == 0 and master_process:
        ...
        if losses['val'] < best_val_loss - early_stopping_min_delta:
            ...
            print(f"⚠ No improvement for {early_stopping_counter}/{early_stopping_patience} checks")
            
            if early_stopping_counter >= early_stopping_patience:
                early_stopping_triggered = True
                print(f"\n{'='*60}")
                print("EARLY STOPPING TRIGGERED")
                print(f"No improvement in validation loss for {early_stopping_patience} evaluations")
                print(f"Best val loss: {best_val_loss:.4f}")
                print(f"Stopping at iteration {iter_num}")
                print(f"{'='*60}\n")
...
if master_process:
    ...
    print(f"Best val loss: {best_val_loss:.4f}")
    if early_stopping_triggered:
        print(f"Stopped early after {early_stopping_counter} checks without improvement")
    else:
